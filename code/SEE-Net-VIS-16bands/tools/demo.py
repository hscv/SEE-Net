...
def track_once(gtArr, video_path_name):
    ...
    save_det_path = 'demo/'+pre_foldName+'/' + video_name+'_det.txt' 
    ...
    for frame in get_frames(video_path_name):
        cnt += 1
        falseColorFrame = falseColorFrameArr[cnt]
        if first_frame:
            # build video writer
            if args.save:
                if video_path_name.endswith('avi') or \
                    ...
                    fps = 30
                save_video_path = 'demo/'+pre_foldName+'/'+video_name+'_tracking.mp4'
            ...
            try:
                ...
                exit()
            print ('cnt = ', cnt+1, ' , bbox = ', gt_arr[cnt], ' gt_arr[cnt] = ', gt_arr[cnt])
            ...
            print ('cnt = ', cnt+1, ' , bbox = ', bbox_f, ' gt_arr[cnt] = ', gt_arr[cnt])
            cv2.rectangle(falseColorFrame, (int(bbox[0]), int(bbox[1])),
                          (int(bbox[0]+bbox[2]), int(bbox[1]+bbox[3])),
                          (0, 255, 0), 2)
            det_arr.append(bbox_f)
            for tmp in bbox_f:
                f.write(str(tmp)+'\t')
            f.write('\n')
