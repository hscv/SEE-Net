...
class SiamBANTracker(SiameseTracker):
    ...
    def _bbox_clip(self, cx, cy, width, height, boundary):
        ...
        width = max(2, min(width, boundary[1]))
        height = max(2, min(height, boundary[0]))
    ...
    def track(self, img, gt):
        ...
        scoreArr = [self._convert_score(oo) for oo in outputs['cls']]
        pred_bboxArr = [self._convert_bbox(loc, self.points) for loc in outputs['loc']]

        order = outputs['order']
        penalty = order[0].mean(dim=0)
        pe_arr = []
        for k in range(len(scoreArr)):
        ...
        widthArr = []
        heightArr = []
        for tmp_i in range(len(scoreArr)):
            pred_bbox = pred_bboxArr[tmp_i]
            score = scoreArr[tmp_i]
            # scale penalty
        ...
        for i in range(len(scoreArr)): 
            va = pe_arr[i].detach().cpu().numpy()
            bbox = bboxArr[i]
            bboxNp = np.array(bbox)
            cx = cxArr[i]
            cy = cyArr[i]
            width = widthArr[i]
            height = heightArr[i]
            if i == 0:
                ...
                widthRes += width*va
                heightRes += height*va
        bboxNpRes = bboxNpRes / sumPeArr
        bbox = bboxNpRes.tolist()
        best_score = best_score / sumPeArr
        cxRes = cxRes / sumPeArr
        cyRes = cyRes / sumPeArr
        widthRes = widthRes / sumPeArr
        heightRes = heightRes / sumPeArr
        # udpate state
        self.center_pos = np.array([cxRes, cyRes])
        self.size = np.array([widthRes, heightRes])
