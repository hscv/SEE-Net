...
def build_opt_lr(model, current_epoch=0):
    ...
    for name, value in model.chanModel.named_parameters():
        value.requires_grad = True

    trainable_params = []

    trainable_params += [{'params': model.head.parameters(),
                           'lr': 0.0001}] # cfg.TRAIN.BASE_LR

    trainable_params += [{'params': model.chanModel.parameters(),
                          'lr': 0.0001}] # cfg.TRAIN.BASE_LR
